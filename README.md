...
```bash
ollama serve
ollama pull gemma4:e4b       # Основная модель перевода
ollama pull qwen3.5:9b       # Вспомогательная (глоссарий, QE, анализ)
```
...
|---|---|---|---|---|
| **Gemma 4 4B** ⭐ | 3 GB | ~8 GB | Рекомендуется: 140 языков, efficient, слабое железо | `gemma4:e4b` |
| **Gemma 4 26B** | 17 GB | ~24 GB | Мощная: максимум качества Gemma | `gemma4:26b` |
| **Gemma 4 31B** | 20 GB | ~32 GB | Топовая Gemma, серверное железо | `gemma4:31b` |
| **Qwen 3.5 9B** | 5 GB | ~12 GB | Отлично для CJK, 201 язык | `qwen3.5:9b` |
| **Qwen 3.5 27B** | 17 GB | ~24 GB | Топ-качество для сложного контента, сарказма, идиом | `qwen3.5:27b` |
...
| `-m`, `--model` | Модель Ollama | `gemma4:e4b` |
...
| `--aux-model` | Вспомогательная модель для анализа / глоссария / QE | `qwen3.5:9b` |
